...
--
-- Identical to the postgresql copy, and deliberately written to be: no DO
-- block, no UPDATE ... FROM, and the updated table referred to by name rather
-- than an alias, all of which H2 and Postgres read the same way. The file is
-- still duplicated because the schema is split by vendor -- a version present
-- in one folder and missing from the other fails startup on drift.
