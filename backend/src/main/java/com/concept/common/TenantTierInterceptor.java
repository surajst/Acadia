...
public class TenantTierInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            com.concept.user.UserRepository userRepository = WebApplicationContextUtils
                    .getRequiredWebApplicationContext(request.getServletContext())
                    .getBean(com.concept.user.UserRepository.class);
            com.concept.tenant.TenantRepository tenantRepository = WebApplicationContextUtils
                    .getRequiredWebApplicationContext(request.getServletContext())
                    .getBean(com.concept.tenant.TenantRepository.class);

            String username = auth.getName();
            Optional<com.concept.user.User> userOpt = userRepository.findByEmail(username);
            if (userOpt.isEmpty()) {
            ...
            com.concept.tenant.Tenant tenant = tenantRepository.findById(tenantId).orElse(null);
            if (tenant == null || tenant.getEffectiveTier() != com.concept.tenant.TenantTier.PARENT_APP_ONLY) {
                return true;
            }
