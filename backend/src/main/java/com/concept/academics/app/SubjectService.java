...
import com.concept.academics.data.GradeSubject;
import com.concept.academics.data.GradeSubjectRepository;
...
public class SubjectService {
    ...
    @Autowired
    private com.concept.shared.data.ClassSectionRepository classSectionRepository;
    ...
    // ─── Flat views for the interface layer ─────────────────────────────────

    public SubjectView createSubjectView(UUID tenantId, UUID academicYearId, String code,
                                         String displayName, String colorHex) {
        return SubjectView.of(createSubject(tenantId, academicYearId, code, displayName, colorHex));
    }

    public List<SubjectView> activeSubjectViews(UUID tenantId) {
        return listActiveSubjects(tenantId).stream().map(SubjectView::of).toList();
    }

    public List<SubjectView> allSubjectViews(UUID tenantId) {
        return listAllSubjects(tenantId).stream().map(SubjectView::of).toList();
    }

    public List<SubjectView> subjectViewsForGrade(UUID tenantId, String gradeName) {
        return listSubjectsForGrade(tenantId, gradeName).stream().map(SubjectView::of).toList();
    }

    /**
     * The distinct grades this school runs, in order. Moved off the controller,
     * which was reaching into ClassSectionRepository itself -- the interface
     * layer does not get to query.
     */
    public List<String> gradeNames(UUID tenantId) {
